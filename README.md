...
状态：M1/M2 基础交付已落地。当前已包含 Python/uv 项目骨架、Typer JSON CLI、SQLite migration runner、默认配置、初始 sources 种子数据，以及 jobs list/show 的存储查询基础。
...
Status: M1/M2 foundation delivered. The repository now includes the Python/uv project skeleton, Typer JSON CLI, SQLite migration runner, default config, seeded initial sources, and storage/query foundations for jobs list/show.
...
Adapter-phase commands still to be completed:
